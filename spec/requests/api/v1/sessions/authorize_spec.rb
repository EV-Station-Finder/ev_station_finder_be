...
RSpec.describe 'User authorization' do
  let(:registration_params) { {
    ...
    let(:login_params) { {email: "hari.seldon@foundation.com",
                          password: "verysecurepassword"}}
    let(:session_response) { JSON.parse(response.body, symbolize_names: true) }
    let(:token)  { session_response[:data][:token] }
    let(:token_params) { {"token": token} }
    let(:incorrect_token_params) { {"token": "wrJhbGciOiJIUzI1NiJ9.eyJ1c2VyX2lkIjoxNDV9.l_pdekbafUTPaz88h1qXUIw_Bg7PJPa_4Eu81AN7yfA"} }
    let(:empty_token_params) { {"token": ""} }
  ...
  describe 'Sad Path' do
    it 'Does not authorize user because token is incorrect' do
      get api_v1_authorize_path, headers: headers, params: incorrect_token_params
    ...
    it 'Does not authorize user because token is empty string' do
      get api_v1_authorize_path, headers: headers, params: empty_token_params
